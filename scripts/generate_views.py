...
LP_VIEW_CLASS = 'o_lp_view'  # sistema de diseño clínico: static/src/scss/lp/_native.scss
...
def styled(model, arch):
    ...
    return arch[:match.end()] + f' class="{classes}"' + arch[match.end():]


def view(xmlid, model, arch):
    arch = styled(model, arch)
...
def guided_button(action):
    """Abre el espacio guiado del Centro de Licitaciones para el registro actual (acción de cliente)."""
    return f'<button name="%({action})d" type="action" string="Vista guiada" class="btn-secondary" icon="fa-compass"/>'
...
<page string="Resumen"><group><field name="partidas_count"/><field name="proveedores_count"/><field name="preguntas_count"/><field name="total_costo"/><field name="total_precio"/><field name="margen_pct"/></group><field name="carta_apoyo_ids" readonly="1"><list><field name="folio"/><field name="partner_id"/><field name="state"/><button name="action_print_carta" string="PDF" type="object"/><button name="action_enviar" string="Enviar al proveedor" type="object" invisible="state != 'borrador'" confirm="Se enviará un correo al proveedor. ¿Continuar?"/></list></field></page></notebook></sheet><chatter/></form>''')
    e += view('view_pregunta_form', 'licitacion.pregunta', '<form><header>' + field('state', 'widget="statusbar"') + '</header><sheet><group><group>' + field('expediente_id', 'options="{\'no_create\': True}"') + field('company_id', 'invisible="1"') + field('partida_id', "domain=\"[('company_ids', 'in', [company_id])]\" options=\"{'no_create': True}\"") + '</group><group>' + fields('autor_id fecha') + '</group></group>' + field('texto', 'placeholder="Redacta la pregunta tal como se enviará a la junta de aclaraciones"') + field('respuesta_portal', 'placeholder="Respuesta oficial publicada por la convocante"') + '</sheet><chatter/></form>')
    e += view('view_costeo_linea_form', 'licitacion.costeo.linea', '<form><sheet><group><group>' + field('expediente_id', 'options="{\'no_create\': True}"') + field('company_id', 'invisible="1"') + field('partida_id', "domain=\"[('company_ids', 'in', [company_id])]\" options=\"{'no_create': True}\"") + field('proveedor_id', 'options="{\'no_create\': True}"') + field('cantidad') + '</group><group>' + fields('costo_unitario precio_unitario total_costo total_precio margen_pct') + field('es_secundaria', 'invisible="1"') + field('recargo_secundaria_pct', 'invisible="not es_secundaria"') + field('currency_id', 'invisible="1"') + '</group></group><p class="text-muted">Margen = (precio − costo) / precio. Una empresa secundaria requiere el costeo previo de la principal y un recargo mayor a su margen.</p></sheet></form>')
...
<record id="action_licitaciones_settings" model="ir.actions.act_window"><field name="name">Ajustes de licitaciones</field><field name="res_model">res.config.settings</field><field name="view_mode">form</field><field name="target">current</field><field name="context">{'module':'licitaciones_publicas'}</field></record>''')
    ...
    # Espacios guiados del Centro de Licitaciones (acciones de cliente en views/hub_views.xml).
    for xmlid, name, target, seq in [('inicio', 'Inicio', 'action_lp_inicio', 0), ('agenda', 'Agenda', 'action_agenda', 1),
                                     ('procedimientos', 'Procedimientos', 'action_lp_procedimientos', 3), ('partidas', 'Renglones', 'action_partidas', 4),
                                     ('expedientes', 'Expedientes', 'action_lp_expedientes', 5), ('incidencias', 'Incidencias', 'action_incidencias', 6)]:
        menus += f'<menuitem id="menu_{xmlid}" name="{name}" parent="menu_root" action="{target}" sequence="{seq}"/>'
    menus += '<menuitem id="menu_cargas" name="Cargas" parent="menu_root" sequence="2"/><menuitem id="menu_carga_nueva" name="Nueva carga" parent="menu_cargas" action="action_lp_carga" sequence="1"/><menuitem id="menu_carga_historial" name="Historial" parent="menu_cargas" action="action_cargas" sequence="2"/><menuitem id="menu_carga_clasica" name="Nueva carga (formulario)" parent="menu_cargas" action="action_carga_wizard" sequence="3"/><menuitem id="menu_config" name="Configuración" parent="menu_root" sequence="90"/>'
    ...
    menus += '<menuitem id="menu_proveedores" name="Proveedores" parent="menu_config" action="action_proveedores" sequence="8"/>'
    menus += '<menuitem id="menu_clasico" name="Vistas clásicas" parent="menu_config" sequence="80"/><menuitem id="menu_procedimientos_clasico" name="Procedimientos" parent="menu_clasico" action="action_procedimientos" sequence="1"/><menuitem id="menu_expedientes_clasico" name="Expedientes" parent="menu_clasico" action="action_expedientes" sequence="2"/>'
    menus += '<menuitem id="menu_settings" name="Ajustes" parent="menu_config" action="action_licitaciones_settings" groups="base.group_system" sequence="90"/>'
